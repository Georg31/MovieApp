...
class CustomRatingView: UIView {
    ...
    private func cofigureRatingView(){
        ...
        let circlePath = UIBezierPath(arcCenter: CGPoint(x: self.bounds.midX,y: self.bounds.midY), radius: CGFloat(20), startAngle: CGFloat(-Double.pi / 2), endAngle: CGFloat(-Double.pi / 2), clockwise: true)
        
        ratingLayer.path = circlePath.cgPath
        
        let circlePath2 = UIBezierPath(arcCenter: CGPoint(x: self.bounds.midX,y: self.bounds.midY), radius: CGFloat(20), startAngle: CGFloat(0), endAngle:CGFloat(Double.pi * 2), clockwise: true)
...
extension CustomRatingView{
    ...
    private func ratingAnimation(){
        ...
        let animationPath = UIBezierPath(arcCenter: CGPoint(x: self.bounds.midX,y: self.bounds.midY), radius: CGFloat(20), startAngle: CGFloat(-Double.pi / 2), endAngle:CGFloat(-1.55 + percentToRadians()), clockwise: true)
    ...
    private func labelTextAnimation() {
        let ratingCount = DispatchWorkItem { [self] in
            for i in stride(from: 0, through: rating, by: 0.1) {
                DispatchQueue.main.async {
                    self.ratingLabel.text = String(format: "%.1f", i)
                }
